...
input group    "───── Screenshot / Trade Replay ─────"
input int      ScreenshotWidth   = 1000;                 // Screenshot kengligi (piksel)
input int      ScreenshotHeight  = 600;                  // Screenshot balandligi (piksel)
input int      ExitLinkWindowSec = 300;                   // Pozitsiya yopilgach necha soniyagacha "EXIT" bosqichi sifatida bog'lanadi
...
// "📸" chart tugmasi nomi va shu tugma orqali "open" bosqichi allaqachon
// yuborilgan pozitsiya ticketlari (shu sessiya davomida) — ikkinchi marta
// bosilganda "during" deb belgilash uchun.
string BTN_SCREENSHOT = "SardorTJ_ScreenshotBtn";
ulong  g_seenOpenTickets[];
...
int OnInit()
{
   ...
   LastDealGVName = "SardorTJ_LastDealTicket_" + IntegerToString(MagicNumber);

   Print("Sardor T.J MT5 Bridge ishga tushdi. Foydalanuvchi manzili: ", UserBaseUrl);
   ...
   // Chart Screenshot + Trade Replay uchun "📸" tugmasini chartga chiqaramiz.
...
{
   EventKillTimer();
   ObjectDelete(0, BTN_SCREENSHOT);
}

//+------------------------------------------------------------------+
//| Chartdagi "📸" tugmasi bosilganda ishga tushadi                   |
//+------------------------------------------------------------------+
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK && sparam == BTN_SCREENSHOT)
   {
      // Tugma "bosilgan" holatda qotib qolmasligi uchun darhol qaytaramiz
      ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_STATE, false);
      ChartRedraw(0);
...
void ProcessCommandDoc(string chunk)
{
   ...
   // "screenshot" buyrug'ida ham symbol/lot talab qilinmaydi — ovozli yordamchi
   // orqali "grafikni screenshot qil" kabi buyruqlar shu yo'l bilan bajariladi.
   if(type == "screenshot")
   {
      ProcessScreenshotCommand(docId, chunk);
      return;
   }
...
//+------------------------------------------------------------------+
//| Jurnaldan (ovozli yordamchi orqali) kelgan "screenshot" buyrug'i. |
//| Ixtiyoriy maydonlar:                                              |
//|  - "timeframe": "M15","M30","H1" va h.k. — berilsa, screenshot   |
//|    olishdan oldin chart shu TF'ga o'tkaziladi (bo'sh bo'lsa joriy |
//|    TF saqlanadi)                                                  |
//|  - "ticket": aniq bir pozitsiya/savdo ticketi — berilsa, avtomatik|
//|    aniqlash o'rniga aynan shu savdoga bog'lanadi                  |
//|  - "stage": "open" yoki "exit" — berilsa, avtomatik aniqlangan    |
//|    bosqichdan qat'i nazar shu bosqich yoziladi                    |
//+------------------------------------------------------------------+
void ProcessScreenshotCommand(string docId, string chunk)
{
   string tfReq    = ExtractStringField(chunk, "timeframe");
   string symReq   = ExtractStringField(chunk, "symbol");
   double ticketD  = ExtractDoubleField(chunk, "ticket");
   ulong  ticketReq = (ulong)ticketD;
   string stageReq = ExtractStringField(chunk, "stage"); // "open" | "exit" | ""

   Print("DEBUG: screenshot buyrug'i qayta ishlanmoqda -> docId=", docId,
         " tf=", tfReq, " symbol=", symReq, " ticket=", ticketReq, " stage=", stageReq);

   MarkCommandProcessing(docId);

   string symbol = (symReq != "") ? symReq : ChartSymbol(0);

   if(tfReq != "")
   {
      ENUM_TIMEFRAMES tf = StringToTimeframe(tfReq);
      if(tf != PERIOD_CURRENT)
      {
         ChartSetSymbolPeriod(0, symbol, tf);
         ChartRedraw(0);
         Sleep(600); // grafik yangi TF bilan qayta chizilishi uchun qisqa kutish
      }
   }

   string ticketStr, stage;
   double price, sl, tp, lot, pnl;

   if(ticketReq != 0)
   {
      // Foydalanuvchi aniq bir savdo ID'sini aytgan — avval ochiq pozitsiyalar
      // orasidan qidiramiz, topilmasa tarixdan (yopilgan savdo sifatida).
      if(PositionSelectByTicket(ticketReq))
      {
         ticketStr = IntegerToString((long)ticketReq);
         price = PositionGetDouble(POSITION_PRICE_CURRENT);
         sl    = PositionGetDouble(POSITION_SL);
         tp    = PositionGetDouble(POSITION_TP);
         lot   = PositionGetDouble(POSITION_VOLUME);
         pnl   = PositionGetDouble(POSITION_PROFIT) + PositionGetDouble(POSITION_SWAP);
         stage = (stageReq != "") ? stageReq : "open";
      }
      else
      {
         ticketStr = IntegerToString((long)ticketReq);
         price = 0; sl = 0; tp = 0; lot = 0; pnl = 0;
         stage = (stageReq != "") ? stageReq : "exit";
         if(HistorySelect(0, TimeCurrent() + 1))
         {
            ...
            for(int i = htotal - 1; i >= 0; i--)
            {
               ...
               if((ulong)posId != ticketReq) continue;
               ...
               else if(en == DEAL_ENTRY_IN && price == 0)
               {
                  price = HistoryDealGetDouble(d, DEAL_PRICE); // "open" so'ralganda shu yetarli
                  lot   = HistoryDealGetDouble(d, DEAL_VOLUME);
               }
            }
         }
   ...
   {
      // Ticket berilmagan — "hozir ochiq turgan ekranni screenshot qil" kabi umumiy
      // buyruq: 📸 tugmasidagi kabi avtomatik aniqlashdan foydalanamiz.
      FindScreenshotContext(symbol, ticketStr, stage, price, sl, tp, lot, pnl);
      if(stageReq != "") stage = stageReq; // foydalanuvchi aniq bosqich so'ragan bo'lsa ustunlik beriladi
   }

   CaptureAndUploadScreenshot(symbol, ticketStr, stage, price, sl, tp, lot, pnl, docId);
}
                         ...
                         const MqlTradeResult &result)
{
   ...
   // TUZATISH: avval faqat magic==MagicNumber (778899) bo'lgan, ya'ni EA/ilova
   // orqali ochilgan savdolar yuborilar edi. Terminalda QO'LDA (F9 orqali)
   // ochilgan savdolar odatda magic=0 bilan keladi va shu shart tufayli
   // butunlay e'tiborsiz qoldirilib, jurnalga hech qachon tushmas edi. Endi
   // magic=0 (qo'lda ochilgan) savdolar ham qabul qilinadi — faqat BOSHQA
   // (uchinchi tomon) botlar/EA'larning magic raqami filtrlanadi.
   if(magic != (long)MagicNumber && magic != 0) return;
...
void ReconcileMissedDeals()
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      long magic = HistoryDealGetInteger(dealTicket, DEAL_MAGIC);
      // TUZATISH: yuqoridagi OnTradeTransaction'dagi izohga qarang — qo'lda
      // (magic=0) ochilgan savdolar ham endi qayta ishlanadi.
      if(magic != (long)MagicNumber && magic != 0) { if(dealTicket > maxTicket) maxTicket = dealTicket; continue; }
...
//+------------------------------------------------------------------+
//| MT5 SCREENSHOT + TRADE REPLAY                                     |
//| Treyder "📸" tugmasini bosganda, joriy chart holatini (aynan      |
//| treyder o'zi qo'ygan TF/zoom bilan) screenshot qilib, tegishli    |
//| Trade ID (pozitsiya ticketi) bilan birga Firestore'ga yuboradi.   |
//| Avtomatik screenshot OLINMAYDI — faqat shu tugma bosilganda.      |
//+------------------------------------------------------------------+

//+------------------------------------------------------------------+
//| "📸" tugmasini chartga chiqarish                                  |
//+------------------------------------------------------------------+
void CreateScreenshotButton()
{
   if(ObjectFind(0, BTN_SCREENSHOT) >= 0) return; // allaqachon bor
   ObjectCreate(0, BTN_SCREENSHOT, OBJ_BUTTON, 0, 0, 0);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_CORNER, CORNER_LEFT_UPPER);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_XDISTANCE, 10);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_YDISTANCE, 20);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_XSIZE, 46);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_YSIZE, 32);
   ObjectSetString(0, BTN_SCREENSHOT, OBJPROP_TEXT, "📸");
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_FONTSIZE, 15);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_BACK, false);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_HIDDEN, true);
   ObjectSetInteger(0, BTN_SCREENSHOT, OBJPROP_ZORDER, 100);
   ObjectSetString(0, BTN_SCREENSHOT, OBJPROP_TOOLTIP, "Screenshot olish (jurnalga saqlanadi)");
   ChartRedraw(0);
}

//+------------------------------------------------------------------+
//| ENUM_TIMEFRAMES -> "H1", "M15" kabi qisqa matn                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| "M15", "H1", "D1" kabi matnni ENUM_TIMEFRAMES ga aylantiradi.     |
//| Tanilmasa PERIOD_CURRENT qaytaradi (chaqiruvchi tomon buni "TF     |
//| o'zgartirilmasin" belgisi sifatida talqin qiladi).                |
//+------------------------------------------------------------------+
ENUM_TIMEFRAMES StringToTimeframe(string s)
{
   ...
   return PERIOD_CURRENT; // tanilmagan matn — TF o'zgartirilmaydi
}

//+------------------------------------------------------------------+
//| Shu ticket uchun "open" bosqichi bu sessiyada avval yuborilganmi  |
//+------------------------------------------------------------------+
bool TicketSeenBefore(ulong ticket)
{
   int n = ArraySize(g_seenOpenTickets);
   for(int i = 0; i < n; i++)
      if(g_seenOpenTickets[i] == ticket) return true;
   return false;
}
void MarkTicketSeen(ulong ticket)
{
   if(TicketSeenBefore(ticket)) return;
   int n = ArraySize(g_seenOpenTickets);
   ArrayResize(g_seenOpenTickets, n + 1);
   g_seenOpenTickets[n] = ticket;
}

//+------------------------------------------------------------------+
//| Screenshot qaysi savdoga (Trade ID) tegishli ekanini aniqlaydi:   |
//| 1) shu symboldagi OCHIQ pozitsiya bo'lsa -> "open" (birinchi      |
//|    marta) yoki "during" (keyingi martalar)                        |
//| 2) ochiq pozitsiya yo'q, lekin ExitLinkWindowSec ichida yopilgan  |
//|    bo'lsa -> "exit"                                               |
//| 3) hech narsa topilmasa -> "unlinked" (baribir saqlanadi)         |
//+------------------------------------------------------------------+
void FindScreenshotContext(string symbol, string &ticketStr, string &stage,
                            double &price, double &sl, double &tp, double &lot, double &pnl)
{
   ulong bestTicket = 0;
   long  bestTimeMsc = -1;
   int total = PositionsTotal();
   for(int i = 0; i < total; i++)
   {
      ...
      if(posMagic != (long)MagicNumber && posMagic != 0) continue; // qo'lda ochilgan (magic=0) ham qabul qilinadi
      long tm = (long)PositionGetInteger(POSITION_TIME_MSC);
      if(tm > bestTimeMsc) { bestTimeMsc = tm; bestTicket = t; }
   }

   if(bestTicket != 0 && PositionSelectByTicket(bestTicket))
   {
      ticketStr = IntegerToString((long)bestTicket);
      price = PositionGetDouble(POSITION_PRICE_CURRENT);
      sl    = PositionGetDouble(POSITION_SL);
      tp    = PositionGetDouble(POSITION_TP);
      lot   = PositionGetDouble(POSITION_VOLUME);
      pnl   = PositionGetDouble(POSITION_PROFIT) + PositionGetDouble(POSITION_SWAP);
      stage = TicketSeenBefore(bestTicket) ? "during" : "open";
      return;
   }

   // Ochiq pozitsiya topilmadi — yaqinda yopilgan (EXIT) deal qidiramiz
   if(HistorySelect(TimeCurrent() - ExitLinkWindowSec, TimeCurrent() + 1))
   {
      int htotal = HistoryDealsTotal();
      ulong    bestDeal = 0;
      datetime bestDealTime = 0;
      for(int i = 0; i < htotal; i++)
      {
         ulong d = HistoryDealGetTicket(i);
         if(d == 0) continue;
         if(HistoryDealGetString(d, DEAL_SYMBOL) != symbol) continue;
         long dealMagic = (long)HistoryDealGetInteger(d, DEAL_MAGIC);
         if(dealMagic != (long)MagicNumber && dealMagic != 0) continue; // qo'lda ochilgan (magic=0) ham qabul qilinadi
         ENUM_DEAL_ENTRY en = (ENUM_DEAL_ENTRY)HistoryDealGetInteger(d, DEAL_ENTRY);
         if(en != DEAL_ENTRY_OUT && en != DEAL_ENTRY_OUT_BY) continue;
         datetime dt = (datetime)HistoryDealGetInteger(d, DEAL_TIME);
         if(dt > bestDealTime) { bestDealTime = dt; bestDeal = d; }
      }
      if(bestDeal != 0)
      {
         long posId = HistoryDealGetInteger(bestDeal, DEAL_POSITION_ID);
         ticketStr = IntegerToString(posId);
         price = HistoryDealGetDouble(bestDeal, DEAL_PRICE);
         sl = 0; tp = 0;
         lot = HistoryDealGetDouble(bestDeal, DEAL_VOLUME);
         pnl = HistoryDealGetDouble(bestDeal, DEAL_PROFIT)
             + HistoryDealGetDouble(bestDeal, DEAL_SWAP)
             + HistoryDealGetDouble(bestDeal, DEAL_COMMISSION);
         stage = "exit";
         return;
      }
   }

   // Hech narsa topilmadi — baribir screenshot saqlanadi, lekin savdoga bog'lanmagan holda
   ticketStr = "0";
   price = SymbolInfoDouble(symbol, SYMBOL_BID);
   sl = 0; tp = 0; lot = 0; pnl = 0;
   stage = "unlinked";
}

//+------------------------------------------------------------------+
//| Faylni to'liq uchli (byte) massivga o'qiydi                       |
//+------------------------------------------------------------------+
bool ReadFileBytes(string filename, uchar &data[])
{
   int handle = FileOpen(filename, FILE_READ | FILE_BIN);
   if(handle == INVALID_HANDLE) return false;
   ulong fsize = FileSize(handle);
   ArrayResize(data, (int)fsize);
   uint got = FileReadArray(handle, data, 0, (int)fsize);
   FileClose(handle);
   return (got == fsize);
}

//+------------------------------------------------------------------+
//| Base64 kodlash (Firestore'ga rasm matn (stringValue) sifatida     |
//| yuborish uchun). Katta fayllarda tez ishlashi uchun natija avval  |
//| uchli massivga yoziladi, oxirida BIR MARTA CharArrayToString      |
//| chaqiriladi (satrga qayta-qayta qo'shish sekin ishlagan bo'lardi).|
//+------------------------------------------------------------------+
string Base64Encode(const uchar &data[], int size)
{
   static string tbl = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
   ...
   while(i + 3 <= size)
   {
      uchar b0 = data[i], b1 = data[i+1], b2 = data[i+2];
      out[oi++] = (uchar)StringGetCharacter(tbl, (b0 >> 2) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, ((b0 << 4) | (b1 >> 4)) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, ((b1 << 2) | (b2 >> 6)) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, b2 & 0x3F);
      i += 3;
   }
   int rem = size - i;
   if(rem == 1)
   {
      uchar b0 = data[i];
      out[oi++] = (uchar)StringGetCharacter(tbl, (b0 >> 2) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, (b0 << 4) & 0x3F);
      out[oi++] = (uchar)'=';
      out[oi++] = (uchar)'=';
   }
   else if(rem == 2)
   {
      uchar b0 = data[i], b1 = data[i+1];
      out[oi++] = (uchar)StringGetCharacter(tbl, (b0 >> 2) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, ((b0 << 4) | (b1 >> 4)) & 0x3F);
      out[oi++] = (uchar)StringGetCharacter(tbl, (b1 << 2) & 0x3F);
      out[oi++] = (uchar)'=';
   }
   return CharArrayToString(out, 0, oi, CP_UTF8);
}

//+------------------------------------------------------------------+
//| "📸" tugmasi bosilganda: chartni screenshot qilib, Trade ID va    |
//| metama'lumotlar (vaqt, symbol, TF, narx, SL, TP, lot, P/L) bilan  |
//| birga Firestore'ning mt5_screenshots collection'iga yuboradi.     |
//+------------------------------------------------------------------+
void TakeAndUploadScreenshot()
{
   ...
   Comment("📸 Screenshot olinmoqda...");
   CaptureAndUploadScreenshot(symbol, ticketStr, stage, price, sl, tp, lot, pnl, "");
}

//+------------------------------------------------------------------+
//| Screenshot olib, Firestore'ga yuklovchi umumiy funksiya. Ham      |
//| "📸" tugmasi (docId="" — natija Comment() orqali ko'rsatiladi),  |
//| ham ovozli yordamchidan kelgan "screenshot" buyrug'i (docId!="" — |
//| natija MarkCommandDone/MarkCommandError orqali jurnalga qaytadi)  |
//| shu funksiyani chaqiradi — takrorlanishning oldini oladi.         |
//+------------------------------------------------------------------+
void CaptureAndUploadScreenshot(string symbol, string ticketStr, string stage,
                                 double price, double sl, double tp, double lot, double pnl,
                                 string docId)
{
   bool isCommand = (docId != "");
   string tfStr = TimeframeToString((ENUM_TIMEFRAMES)ChartPeriod(0));

   string filename = "SardorTJ_shot_" + IntegerToString(GetTickCount()) + ".png";
   bool shotOk = ChartScreenShot(0, filename, ScreenshotWidth, ScreenshotHeight);
   if(!shotOk)
   {
      Print("DEBUG: ChartScreenShot muvaffaqiyatsiz. Xato=", GetLastError());
      if(isCommand) MarkCommandError(docId, "Screenshot olinmadi (xato " + IntegerToString(GetLastError()) + ")");
      else Comment("✗ Screenshot olinmadi (xato ", GetLastError(), ")");
      return;
   }
   ...
   {
      Print("DEBUG: Screenshot faylini o'qib bo'lmadi: ", filename);
      if(isCommand) MarkCommandError(docId, "Screenshot faylini o'qib bo'lmadi");
      else Comment("✗ Screenshot faylini o'qib bo'lmadi");
      FileDelete(filename);
      return;
   }

   string b64 = Base64Encode(fileData, ArraySize(fileData));

   // Firestore hujjat hajmi cheklovi (1 MiB) tufayli, base64 juda katta
   // bo'lsa yuklamaymiz — aks holda Firestore xato qaytarib, screenshot
   // butunlay yo'qolib qolardi. Bunday holatda ScreenshotWidth/Height ni
   // Inputs bo'limidan kamaytirish kerak.
   if(StringLen(b64) > 700000)
   {
      Print("DEBUG: Screenshot juda katta (", StringLen(b64), " belgi base64), yuklanmadi.");
      if(isCommand) MarkCommandError(docId, "Screenshot juda katta - ScreenshotWidth/Height ni kamaytiring");
      else Comment("✗ Screenshot juda katta — Inputs'da ScreenshotWidth/Height ni kamaytiring");
      FileDelete(filename);
      return;
   }
   ...
   string url = UserBaseUrl + "/mt5_screenshots";
   string fields = "";
   fields += "\"tradeTicket\":{\"stringValue\":\"" + ticketStr + "\"},";
   fields += "\"symbol\":{\"stringValue\":\"" + JsonEscape(symbol) + "\"},";
   fields += "\"timeframe\":{\"stringValue\":\"" + tfStr + "\"},";
   fields += "\"stage\":{\"stringValue\":\"" + stage + "\"},";
   fields += "\"price\":{\"doubleValue\":" + DoubleToString(price, 5) + "},";
   fields += "\"sl\":{\"doubleValue\":" + DoubleToString(sl, 5) + "},";
   fields += "\"tp\":{\"doubleValue\":" + DoubleToString(tp, 5) + "},";
   fields += "\"lot\":{\"doubleValue\":" + DoubleToString(lot, 2) + "},";
   fields += "\"pnl\":{\"doubleValue\":" + DoubleToString(pnl, 2) + "},";
   fields += "\"capturedAt\":{\"integerValue\":\"" + IntegerToString(nowMs) + "\"},";
   fields += "\"imageBase64\":{\"stringValue\":\"" + b64 + "\"}";
   string body = "{\"fields\":{" + fields + "}}";

   string resp = HttpRequest("POST", url, body);
   FileDelete(filename);

   if(resp == "" || StringFind(resp, "\"error\"") != -1)
   {
      Print("DEBUG: Screenshot yuklashda xato. Javob: ", resp);
      if(isCommand) MarkCommandError(docId, "Screenshot yuklanmadi (internet/Firestore xatosi)");
      else Comment("✗ Screenshot yuklanmadi (internet/Firestore xatosi)");
      return;
   }

   if(stage == "open" && ticketStr != "0") MarkTicketSeen((ulong)StringToInteger(ticketStr));

   Print("DEBUG: Screenshot muvaffaqiyatli yuklandi. stage=", stage, " ticket=", ticketStr);

   if(isCommand)
      MarkCommandDone(docId, (ulong)StringToInteger(ticketStr));
   else
      Comment("📸 Screenshot saqlandi (" + stage + ")");
}
