                            ...
                            double &price, double &sl, double &tp, double &lot, double &pnl)
{
   ...
   if(HistorySelect(TimeCurrent() - ExitLinkWindowSec, TimeCurrent() + 1))
   {
      ...
      for(int i = 0; i < htotal; i++)
      {
         ...
         if((long)HistoryDealGetInteger(d, DEAL_MAGIC) != (long)MagicNumber) continue;
...
void TakeAndUploadScreenshot()
{
   ...
   if(!shotOk)
   {
      ...
      Comment("✗ Screenshot olinmadi (xato ", GetLastError(), ")");
      return;
   }
   ...
   if(!ReadFileBytes(filename, fileData))
   {
      ...
      Comment("✗ Screenshot faylini o'qib bo'lmadi");
      FileDelete(filename);
      return;
   }
   ...
   if(StringLen(b64) > 700000)
   {
      ...
      Comment("✗ Screenshot juda katta — Inputs'da ScreenshotWidth/Height ni kamaytiring");
      FileDelete(filename);
      return;
   }
   ...
   if(resp == "" || StringFind(resp, "\"error\"") != -1)
   {
      ...
      Comment("✗ Screenshot yuklanmadi (internet/Firestore xatosi)");
      return;
   }
